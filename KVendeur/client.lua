...
Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('garagearme', 'main'), true, true, true, function() 
            ...
            RageUI.ButtonWithStyle("FMJ", "Pour sortir une FMJ.", {RightLabel = "→"},true, function(Hovered, Active, Selected)
            if (Selected) then
            Citizen.Wait(1)  
            spawnuniCar("fmj")
            end
        ...
        RageUI.ButtonWithStyle("Contender Max 4X4",  "Pour sortir un Contender.", {RightLabel = "→"},true, function(Hovered, Active, Selected)
            if (Selected) then
            Citizen.Wait(1)  
...
function spawnuniCar(car)
    ...
    local plaque = "LINA"..math.random(1,9)
